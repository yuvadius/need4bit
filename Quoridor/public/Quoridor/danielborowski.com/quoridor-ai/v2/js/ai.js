    ...
    function simulation(wallSimulations,moveSimulations) {
            ...
            for (var t=0;t<wallSimulations.length;t++) {  
                if (inNo[0].indexOf(wallSimulations[t])!=-1) {
                    ...
                    if (whatWallPlace=="wallPlacementHoriz") {  
                        ...
                        var checkLegal = shortestPath(inNo[1]+","+test_wall_1+","+test_wall_2,null,null);
                        if (checkLegal[1]==0 && checkLegal[0]!=0) { 
                            wallPreventTrap = wallSimulations[t]; 
                            var actuallyLegal = pathToEndExists(wallPreventTrap+","+test_wall_2,null,null);
                            if (!actuallyLegal) { wallPreventTrap = null; }
                            else { console.log("SINGLE TRAP: "+wallSimulations[worstWallForComp]+","+wallSimulations[t]); break; }      
        ...
        arrShortestMoves_b[1]-arrShortestMoves_b[0]<=-2*/) {   
            ...
            var finalLocToCheck = null;
						var nextLoc;
						var s = compLoc-1;
            var fromLoc = wallSimulations.indexOf(s+"-"+compLoc); 
            if (fromLoc==-1) {
								var ss = compLoc+1;
                nextLoc = wallSimulations.indexOf(compLoc+"-"+ss);
                if (nextLoc==-1) {
										var sss = compLoc-9;
                    nextLoc = wallSimulations.indexOf(sss+"-"+compLoc); 
                    if (nextLoc==-1) { finalLocToCheck = 10; } // lol idk wtf
                    else { finalLocToCheck = nextLoc;  }           
                } else { finalLocToCheck = nextLoc; }
            } else { finalLocToCheck = fromLoc; } 
						//console.log("...... "+compLoc);
						//console.log(",,,,,,, "+finalLocToCheck);
						//console.log("'''''''' "+wallSimulations);
            // find player double blocking walls
            for (var t=finalLocToCheck+45;t>finalLocToCheck-45;t--) { // CHANGED FROM 35   
                if (wallPreventTrap==null && typeof wallSimulations[t]!='undefined') {  
                    ...
                    if (checkLegal!='illegal' && typeof checkLegal!='undefined' && typeof checkLegal[0]!='undefined' && checkLegal[0]!='' && checkLegal[0]!=' ') {  
                        for (var a=finalLocToCheck+45;a>finalLocToCheck-45;a--) { // CHANGED FROM 35     
                            if (test_wall_1!=wallSimulations[a] && test_wall_2!=wallSimulations[a] && wallPreventTrap==null &&
                                ...
                                if (checkLegal_2!='illegal' && typeof checkLegal_2!='undefined') {        
                                    ...
                                    // MESS WITH VALUE HEREEEEEEEE!!!
																		if (sp_t[1]>=low_change_computer+4) {
                                        ...
                                        for (var b=finalLocToCheck-90;b<finalLocToCheck+20;b++) { // CHANGED FROM 90, 10   
                                            ...
                                            test_wall_1_1!=wallSimulations[b] && test_wall_2_2!=wallSimulations[b] && typeof wallSimulations[b]!='undefined') {  
                                                ...
                                                if (checkLegal_3!='illegal' && typeof checkLegal_3!='undefined') { 
                                                    for (var c=finalLocToCheck-90;c<finalLocToCheck+20;c++) { // CHANGED FROM 90, 10   
            ...
            for (var p=0;p<playerMoves_2[1].length;p++) { 
                ...
                if (temp_sp[0]<move_sp) {
                    ...
                    tempLoc_2 = p;       
						...
						console.log("CHECKING FOR FUTURE MOVE!!!!!: "+move_sp, tempLoc_1, playerMoves_1[1], tempLoc_2, playerMoves_2[1]);
        ...
        var finalLocToCheck = null;
				var q = playerLoc-1;
        var fromLoc = wallSimulations.indexOf(q+"-"+playerLoc); 
        if (fromLoc==-1) {
						var qq = playerLoc+1;
            var nextLoc = wallSimulations.indexOf(playerLoc+"-"+qq);
            if (nextLoc==-1) {
								var qqq = playerLoc-9;
                nextLoc = wallSimulations.indexOf(qqq+"-"+playerLoc); 
                if (nextLoc==-1) {  
										var qqqq = playerLoc+9;
                    nextLoc = wallSimulations.indexOf(playerLoc+"-"+qqqq);
                    if (nextLoc==-1) {
                        finalLocToCheck = 50; // random number
                        console.log("CANT ATTEMPT DOUBLE WALL",finalLocToCheck);        
                    } 
                } else { finalLocToCheck = nextLoc; }     
            } else { finalLocToCheck = nextLoc; }
        } else { finalLocToCheck = fromLoc; } 
        ...
        // determine DOUBLE wall that slows player if computer is behind at least 2 spaces (takes ~ 4 seconds) // NOT BEHIND ALWAYS DO
        if (finalLocToCheck!=null) {
            for (var t=finalLocToCheck-35;t<finalLocToCheck+35;t++) { // CHANGED FROM 25 
                if (doublePlaceWall==null && compWallsLeft>1 && typeof wallSimulations[t] != 'undefined' && playerWallsLeft-compWallsLeft<4 && 
                arrShortestMoves_b[1]-arrShortestMoves_b[0]>=1) {     
                    var whatWallPlace = $('#board td[data-pos='+wallSimulations[t]+']').attr('class');    
                    if (whatWallPlace=="wallPlacementHoriz") {  
                        var test_wall_1 = wallSimulations[t];
                        var test_wall_2 = $('#board td[data-pos='+(test_wall_1)+']').next().next().attr('data-pos');
                    ...
                    if (checkLegal!='illegal') { 
                        for (var a=finalLocToCheck-35;a<finalLocToCheck+35;a++) {  
                            if (checkLegal[0].indexOf(wallSimulations[a])!=-1) {
                                ...
                                if (checkLegal_2) {       
																		//console.log(test_wall_1,test_wall_2,test_wall_1_1,test_wall_2_2);
                                    ...
                                    if (checkSP2walls[0]>high_change_player/*+1*/ && checkSP2walls[1]<=low_change_computer+1) { 
																				console.log("POSSIBLE DOUBLE WALLS: "+test_wall_1+","+test_wall_2+","+test_wall_1_1+","+test_wall_2_2);
                                        doublePlaceWall = a; 
        ...
        console.log("?? SHORTEST PATHS: "+arrShortestMoves_b);  
				console.log("?? TRAP POSSIBLE: " + wallPreventTrap);
				console.log("?? DOUBLE WALL POSSIBLE: "+doublePlaceWall);
				console.log("?? FUTURE WALL POSSIBLE: " + futureWall);
        
        // split board up - heuristic-ish for stopping long paths
        if (ranWallMove<0.0 && playerWallsLeft>8 && compWallsLeft>8 && 
															 (wallSimulations.indexOf("20-21")!=-1 && wallSimulations.indexOf("21-22")!=-1 && 
                                wallSimulations.indexOf("22-23")!=-1 && wallSimulations.indexOf("23-24")!=-1)) {      
            var r = Math.random();
						if (r<0.25) { var p = "20-21"; } 
            else if (r<0.5) { var p ="21-22"; } 
            else if (r<0.75) { var p = "22-23"; } 
            else { var p = "23-24"; } 
            placeVertWall(p); 
						console.log("MOVE CHOSEN: RANDOM WALL");
        }
        ...
        high_change_player-arrShortestMoves_b[0]<6 && !futureWall && playerWallsLeft!=10 && compWallsLeft!=10 /*&& playerWallsLeft>0*/ 
				&& playerWallsLeft-compWallsLeft<2 && arrShortestMoves_b[1]-arrShortestMoves_b[0]>=-1) 
				{ 	
            console.log("MOVE CHOSEN: DOUBLE PLACE WALL: "+wallSimulations[doublePlaceWall]);      
        ...
        // only check for traps if computer is losing by at most X spaces and if winning at all 
        else if (wallPreventTrap!=null && arrShortestMoves_b[1]-arrShortestMoves_b[0]<3 && high_change_player-arrShortestMoves_b[0]<6 && 
				playerWallsLeft-compWallsLeft<3) {
            console.log("MOVE CHOSEN: COMPUTER TRAP WALL: "+wallPreventTrap); 
            ...
            if (arrShortestMoves_b[1]-arrShortestMoves_b[0]>=1 && arrShortestMoves_a[bestWall_sp][1]-arrShortestMoves_b[1]<=getDiff) {
                console.log("MOVE CHOSEN: WALL IS OK FOR COMPUTER (LOSING)");
            ...
            } else if (arrShortestMoves_b[1]-arrShortestMoves_b[0]<1) {
                console.log("MOVE CHOSEN: WALL IS OK FOR COMPUTER (NOT LOSING)")
                var whatWallPlace = $('#board td[data-pos='+(bestWall)+']').attr('class'); 
                if (whatWallPlace=="wallPlacementHoriz") { var wall = placeHorizWall(bestWall); }
                else if (whatWallPlace=="wallPlacementVert") { var wall = placeVertWall(bestWall); }     
            } else {
                console.log("MOVE CHOSEN: WALL NOT HELPFUL FOR COMPUTER SO JUST MOVE");
